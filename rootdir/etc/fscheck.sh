...
FS_SYSTEM=$(eval $(/sbin/ext/blkid /dev/block/mmcblk0p25 | $BB cut -c 24-); $BB echo $TYPE);
FS_CACHE=$(eval $(/sbin/ext/blkid /dev/block/mmcblk0p26 | $BB cut -c 24-); $BB echo $TYPE);
FS_DATA=$(eval $(/sbin/ext/blkid /dev/block/mmcblk0p28 | $BB cut -c 24-); $BB echo $TYPE);
